...
| 诊断日志 | 默认关闭；可在设置中手动开启，将关键生命周期、页面识别、文件夹读写和降级路径导出为结构化 JSON。 |
...
诊断日志默认不记录。需要排查问题时，可在 popup 或 side panel 的“设置”中手动开启，随后导出 `deepseek-enhancer.logs.v1` 格式的 JSON；日志存储在 `dse.diagnosticLogs.v1`，最多保留最近 500 条事件。日志调用不会主动记录聊天正文、会话标题或完整 URL。
